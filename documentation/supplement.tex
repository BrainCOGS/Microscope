...
\newline\newline
This supplemental material to the article ``TWINKLE: An open-source two-photon microscope for teaching and research'' describes the assembly, function and alignment of all parts of the microscope. Technical specifications are provided through the CAD files and the Zemax simulations in the online repository.
...
\begin{itemize}
    \item Use a high quality duster to remove any residue from optical surfaces. We use a Miller-Stephenson Aero-Duster Ultra MS-222L.
...
All custom metal pieces are parts of the CAD assembly of the microscope. Ours were manufactured from Aluminium stock in the university machine shop, but other materials are possible. Aluminium has the added benefit of anodization, which can help control stray light. For example, if the microscope was used in a Virtual Reality system, we would anodize the collection optics to reduce scattering from the projector's light.\newline
An overview of the microscope's cost is shown in Tab.~\ref{tab_costs}. Keep in mind that these prices can significantly change over time, while some products can become unobtainable and need to be replaced with a different or newer model.
%
\begin{table}
\begin{center}
...
\rowcolor{gray!20} 920 nm fs-pulsed Laser  & Spark Alcor 920 + Warranty & 90 k \\
                   ScanImage DAQ-System \& optimized PC & Vidrio & 20 k \\
...
\begin{figure}[!t]
    ...
    \caption{{\bf Overall view of the beam conditioning path on the optical table.} Shown is a photograph of the components used to condition the beam for our microscope. Key components of the system are labelled with capital letters. The laser is shown as a red transparent line with intensity color coded. The beamsplitter cube (component D) splits the beam into two, halving its power. The Pockels cell (component G) reduces the power further. The reflections out of the Pockels cell and from the shutter (item J) are shown in yellow. The telescope (item K) magnifies the beam $\times 2$. All parts are explained in the text together with a note on their alignment.}
    \label{sfig1}
\end{figure}
%
\begin{enumerate}[]
    ...
    \item \textbf{H)} Two beam blocks (LB1, Thorlabs). Confirm that the right beam block terminates the waste beam leaving the Pockels cell. The left beam is used for calibrating the Pockels cell in ScanImage (the control software used by our microscope \cite{Vidrio2022, Pologruto2003}), together with a light sensor (I).

    \item \textbf{I)} Si Switchable Gain Detector, 350 - 1100 nm (PDA36A2, Thorlabs). This is used to measure the light transmission through the Pockels cell, by monitoring scattered light from the beam block (H), that comes from the shutter back reflection. Make sure that the gain on the detector is chosen reasonably for laser power and ambient light level. In our system, a gain of 50 dB works well. If the ambient room light is too bright, a SM1-mounted infrared long-pass filter in front of the photodiode can help (e.g. FELH0850, Thorlabs). The goal of setting the gain is well-defined sigmoidal control curve in ScanImage: Light transmission through the Pockels cell starts at a minimum, and increases as the Pockels control voltage increases.
        
    \item \textbf{J)} 6 mm Shutter (LS6S2ZM1, Vincent Associates) connected to driver (VCM-D1, Vincent Associates) for control of the shutter during imaging. To check that the shutter is properly aligned leave it open and make sure that the beam is not clipped. When closed, the back-reflection has to be terminated in a beam block. This back-reflection combined with a physically close detector (Items H and I above) is used to calibrate the Pockels cell in ScanImage (see below).

    \item \textbf{K)} $2\times$ Beam Expander ($2\times$ Achromatic Galilean Beam Expander, GBE02-B, Thorlabs, inside SM1RC, Thorlabs). This expands the beam sufficiently to fill the apertures of the subsequent optics. Aligning the beam expander requires two steps. First, it has to be aligned to the optical axis. Second, it is critical that the beam stays collimated after the beam expander. For the first alignment, we use two irises, one screwed in the front, and one (on a reasonably long SM1 extension tube) behind the expander. The beam expander has to be positioned such that the irises cut into the beam symmetrically. Then, the expander is on-axis. Next, we need to adjust the collimation of the beam so that the beam is the same size at the exit and after a long distance using several mirrors, see Fig.~\ref{sfig3}. For optimal alignment, adjust the dial on the beam expander until it no convergence or divergence is visible.
    %
    \begin{SCfigure}
    ...
    \end{SCfigure}
    %
\item \textbf{L)} Iris (SM1D12C inside CP08, both Thorlabs) Used to check alignment.

\item \textbf{M)} Mirror (PF10-03-P0, Thorlabs, inside KCB1) to reflect the beam up at a right angle. For the next steps (in particular finding the optimal scanning mirror position), it is important that the beam travels up perpendicular to the optical table, and parallel to the 95 mm optical rail column that holds the scanning mirrors (next section).

\end{enumerate} 
%
When finished aligning the beam through all the optical components listed above, clamp them down on the table securely. Then shield the beam path with additional SM1 and SM2 tubing to increase safety and to protect the aligned optical components.
%
\subsection{Assembly of the scanning post}

\subsubsection{Components}

Following beam conditioning on the optical table, the beam is reflected up from the optical table towards the ceiling into a box housing the scanning mirrors. This is shown in Fig.~\ref{sfig4}. Following the scanning mirrors, the beam travels up through the scan lens, and directed to the microscope head with another mirror.
%
\begin{figure}[h]
...
\begin{enumerate}[]
    ...
    \item \textbf{C)} An aluminium box containing the resonant and slow galvanometer set (CRS8K/6215H Galvo Set, Novanta), mounted to 95 mm rail (XT95RC4, Thorlabs). A detailed description is provided below.
    \item \textbf{D)} Empty tubing.
    \item \textbf{E)} Scan lens - See detailed description below.
    \item \textbf{F)} Mirror in its housing (PF20-03-P01 and KCB2, both Thorlabs). While we used a round 2 inch mirror, this should be replaced with an elliptical mirror and housing (PFE20-P01 and KCB2EC, both Thorlabs). This increases the field-of-view because the cross-section of an ellipse at 45 degrees is a circle. At low magnification, a round mirror will lead to vignetting.
...
%
\begin{enumerate}[]
    \item \textbf{A)} Empty tube, shielding the bream aligned in the previous section.
...
\begin{itemize}
    ...
    \item Install the collection mirror (component F in Fig.~\ref{sfig5}). Then position an iris in place of the future objective, and one as far away as possible just above the optical table. Shift the mirror along the rail and adjust its tilt via the set screws so that the beam is aligned with the two irises.
\end{itemize}
...
PMTs are light sensitive. Always store them with their aperture covered with black tape when not in use. When removing the tape for installation, this installation into the collection box must be done in darkness and as quickly as possible. We recommend to use black silicone putty (V-tie self setting silicone rubber), and have it ready for use to seal the gap between the PMTs and the aluminium box. Switch the room lights off, wait a few minutes to adapt to the darkness then mount the PMTs on their respective ports.

\subsection{Shield the light path}
Now that your microscope is ready, finish shielding the light path with SM1 and SM2 tubing, and cover the optical table with sheet metal. We use bent Aluminium sheet, placed on small columns made from T-slotted rail.
...
\begin{itemize}
    \item Place the first element on top of the spanner wrench (Thorlabs SPW604) using lens paper. Check the lens thoroughly. Dust can be blown away with a duster, finger prints wiped off with Methanol and a lens cleaning tissue (cf. Fig.~\ref{sfig8}B).
...
\begin{figure}[h]
    ...
    \caption{{\bf Assembly of the collection optics.} \textbf{A)} The assembled collection optics with bottom cover removed, perspective from below. \textbf{B)} The collection optics during the assembly process, similar to panel A. Key components are labelled (see text).}
    \label{sfig9}
\end{figure}
%
\begin{enumerate}[]
    ...
    \item \textbf{G)} With the condensers in place, we can install the optical band-pass filters in front of them. First the 525 nm band-pass filter (Semrock Brightline FF01-525/45-32, AVR Optics) for the green channel, which is secured with a nylon set screw (stainless steel nylon-tip set screw, 4-40 thread, 1/8in long). Orientation is important: The arrow is facing outwards, i.e. away from the inside of the box and towards the PMT along the light path.
    ...
    \item \textbf{L)} PMT plate. The PMT attaches to the FLAT side of the PMT plates. The other side has a shallow cut out for the screw head to sit. The plate with the PMT attached screws into the rest of the assembly using 4-40 5/16 inch, socket cap screws.
...
\begin{figure}[!t]
    ...
    \caption{{\bf The scanning mirror box.} \textbf{A)} The custom aluminium box housing the pair of scanning mirrors. \textbf{B)} Close-up of the same box with key parts labelled: A: Two broadband windows (WG11010-B, Thorlabs) to keep dust out. B: Attachment of the box to the 95 mm optical rail. C: An Iris used for alignment, and D: SM1 tubing. \textbf{C)} Bottom part of the aluminium box. A: The windows, and B: screws to attach the box to the drop-on rail carriage via four 0.75 inch tall 1 inch diameter posts. \textbf{D)} The scanning mirrors are mounted to the lid using heat transfer compound. E: Anodized aluminium block holding the scanning mirrors. F: One of the scanning mirror assemblies, G: The aluminium lid and H: The scanning mirrors. \textbf{E)} Same as \textbf{D} but different angle. \textbf{F)} Same as \textbf{E} but different angle. I: Cables controlling the scanning mirrors.}
    \label{sfig10}
\end{figure}
%
The scanning mirror housing is a machined piece of aluminium holding the Novanta scanning mirror set against the 95 mm optical rail, see Fig.~\ref{sfig10}. It also shields the scanning mirrors from dust in the environment, and protects the environment from audible noise produced by the scanning mirrors. The housing is composed of a box, mounted to the 95 mm rail (Fig.~\ref{sfig10}A), and a lid to which the Novanta scanners are mounted (Fig.~\ref{sfig10}B,C). When mounting the black anodized holder, make sure to apply heat transfer compound between the holder and the lid (we use Wakefield Thermal Joint Compound Type 120-5 Silicone). While the resonant scanner does not dissipate much heat, the slow galvanometer becomes warm in normal operation, and this heat needs to be dissipated. This is particularly important when pushing to fast responses, and driving the slow galvanometer with larger currents.\newline
After receiving the scanning mirrors, they need to be installed in the black anodized Aluminium block. Make sure to put the resonant scanner in first (the piece sitting at an angle) before the slow galvanometer (the piece sitting parallel to the mount). Both pieces should have stops that keep you from pushing them into the mount too far.\\
The box has two windows that are SM1 threaded. Position the dust filters (WG11010-B, Thorlabs) in these bores with appropriate retaining rings. We used a SM1 to SM2 converter ring at the exit, Fig.~\ref{sfig10}B, only because we had that part available from an earlier project.
...
\end{figure}
%
Fig.~\ref{sfig11}A shows as the housing box (component A) and xy scanning mirror set (component B) that we discussed above. Components C are the cables connecting the scanning mirrors to their driver boards. These driver boards are housed in a yellow prototype box, and connected to accept power (component D), and signals (component F). Fig.~\ref{sfig11}B shows the power supply of the scanning mirrors. The slow galvanometer needs a $\pm 28\text{ V}$ supply. This is provided through the three black/white/red cables (-28 V, 0 V, +28 V) from the right power supply. The second power supply to the left provides 12 V for the resonant scanner. The supplies are connected to the power supply ports on the prototype box, and the signal lines are connected to the Vidrio DAQ system. Key components in the figure are labelled with capital letters. These are as follows:
\begin{itemize}
    \item \textbf{A)} Power supply (GPD-3303D, Gw Instek) with two channels, both set to 28 V.
    ...
    \item \textbf{I)} The +28 V output line (red) connected to the +28 V line on the electronics box.
    \item \textbf{J)} Power supply (E3630A, Keysight) set to 12 V to power the resonant scanners.
    \item \textbf{K)} Black channel is the references, and connected to black port on the controller box.
    \item \textbf{L)} Red channel, set to +12 V, is connected to the red port on the controller box.
...
\end{figure}
%
The first components to be wired up are the Murata DMS-20PC displays. The circuit diagram is shown in the main manuscript \textcolor{red}{Fig. 4}.
\begin{itemize}
...
\begin{itemize}
    ...
    \item Leave everything under the \texttt{Triggers}, \texttt{Advanced}, and \texttt{I2C} tabs unchanged.
    \item Of note: (1) Under \texttt{Advanced}, \texttt{Reverse line read} \textcolor{red}{Edward states ``reverses the image. We talked about this in the video that Esme’s scope needs this''. Why?} (2) The setting \texttt{I2C} is used for syncing with behavior.
    \item Click \texttt{Apply}.
\end{itemize}
%
...
\begin{itemize}
    ...
    \item Every couple of month, switch the red and green PMTs to check for signs of aging and validate their performance on a fluorescent slide or sample of known brightness. The PMTs age and will need to be replaced, depending on use, after a few years. They should be treated with care: power ramped up and down slowly, and saturation should be avoided. We use a rotation system in which new PMTs are typically used for the green channel. When their performance becomes worse, they often have a few more months or years in the (typically brighter) red channel in them.
    \item Every couple of month, image a bead sample to validate the PSF.
    \item Be mindful of imaging artifacts. When suspecting an issue, go back to the basics, and isolate the error.
\end{itemize}
%
Of note, for virtual reality experiments that involve a Styrofoam treadmill floating on an air cushion, or any other experiment using compressed air, make sure to place an oil trap in the air line. This avoids oil droplets being deposited onto the optics; especially the elliptic mirror right above the objective. Small Styrofoam particles also get deposited on the lenses which need to be dusted from time to time. Try to shield the laser path as much as possible to keep dust and dirt away from optical components.
